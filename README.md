...
### 2026-09-02 — Frontend PHP conversion: remaining 9 pages (Section 13)
- **What:** Converted the remaining 9 pages to the shared-partial `.php` pattern proven in the Section 13 proof of concept. **Public (6 total now):** `index.php`, `blog.php`, `category.php`, `contact.php`, `post.php` added alongside the existing `about.php`, all requiring `partials/{header,footer}.php` with their own `$pageTitle`/`$pageDescription`/`$activeNav`. `index.php` additionally passes `$extraHead` for its `og:image`/`twitter:card` meta tags (built as a plain string, not heredoc, to match the original's exact indentation byte-for-byte). **Dashboard (5 total now):** `dashboard/index.php`, `dashboard/posts.php`, `dashboard/post-editor.php`, `dashboard/media.php` added alongside the existing `dashboard/categories.php`, all requiring `dashboard/partials/{header,footer,sidebar}.php` with their own `$pageTitle`/`$activeSidebar`/`$topbarTitle`/`$topbarSubtitle`/`$topbarActionHtml`. One small backward-compatible addition to `dashboard/partials/header.php`: an optional `$topbarTitleId` variable that, when set, adds an `id=""` to the topbar `<h1>` — needed only by `post-editor.php` (its own inline script swaps "New Post"/"Edit Post" via `getElementById('editor-page-title')`), left unset (no `id` attribute rendered, identical to before) on every other page.
- **Where:** New files `index.php`, `blog.php`, `category.php`, `contact.php`, `post.php`, `dashboard/index.php`, `dashboard/posts.php`, `dashboard/post-editor.php`, `dashboard/media.php`. Edited `dashboard/partials/header.php` (added the optional `$topbarTitleId` param, backward-compatible). Original `.html` files for all of these left untouched for now — the `.html` → `.php` rename/redirect and internal link updates are a separate not-yet-done task below.
- **Why:** This is the "convert the remaining pages" task the proof of concept was gating. Same partial boundaries, same per-page variable contract, no new patterns introduced — just repeating what `about.php`/`dashboard/categories.php` already proved out.
- **Tested:** Live tested. Every new `.php` file's CLI output diffed byte-for-byte against its original `.html` (`diff foo.html <(php foo.php)`) — zero-diff on all 9 after one whitespace fix in `index.php`'s `$extraHead`. Re-confirmed over live HTTP with MariaDB + PHP's built-in server (`bin/dev-router.php`): all 11 converted pages (including the two from the POC) return `200` and their response bodies are still byte-identical to the original `.html` files, fetched and diffed live, not just via CLI. Checked every page's PHP output under `error_reporting=E_ALL` — no warnings/notices/deprecated output on any of them.
...
- [x] Dashboard is now protected — Section 11 added `dashboard/login.html` and wired every dashboard page to `/auth/*` via `requireDashboardAuth()`, replacing the hardcoded "Sarah Chen" topbar with the real logged-in user
...
- [x] Public frontend wired to API — `assets/js/app.js`, new `assets/js/api.js`
  - **What:** New `assets/js/api.js` is the shared `fetch()` wrapper (`Api.get/post/patch/del/upload`) — unwraps the `{success,data,meta}`/`{success:false,error,errors}` envelope from `core/Response.php` into a resolved value or a thrown `Error` (`err.status`, `err.errors`), so callers never touch `res.ok`/`res.json()` directly. `app.js` is rewritten end-to-end: Home fetches `/posts`, Blog archive does server-side search/category/pagination (category filter values switched from the old mock category *id* to the *slug* `PostRepository::publishedPaginated()` actually filters on), Category page fetches `/categories/{slug}` + `/posts?category=`, Post page fetches `/posts/{id}` (numeric id — post links switched from `post.slug` to `post.id` since `PostController::show()` only accepts an id), records a view via `POST /posts/{id}/view`, and renders the embedded `comments` array. The comment form gained name/email inputs (the API requires `author_name`/`author_email`, which the old demo-only textarea never collected) and now posts to `POST /posts/{id}/comments`. `components.js`'s `Card.post()` was adapted to the real Post shape — no author avatar or `readTimeMinutes` field exists on the backend, so the avatar image was dropped and read time is now estimated client-side (~200wpm). Contact form and newsletter forms stay demo-only (no backend endpoint exists for either). The About page's team section stays on `MOCK_AUTHORS` too — there's no team/author-bio API, that's static site content, not blog data.
  - **Where:** `assets/js/api.js` (new), `assets/js/app.js`, `assets/js/components.js` (`Card.post`), `post.html` (comment form markup), `assets/css/style.css` (`.comment-form input` rules), `blog.html` (removed the inline mock-category-populating script — `app.js`'s `initBlog()` now populates it from `/categories`), every public HTML page (added the `api.js` `<script>` tag).
  - **Why:** This is the first thing actually consuming the five modules built in Section 9 — wiring it exposed the real API contract (numeric-id-only post lookup, slug-based category filtering, no tags/avatar/read-time fields) that the mock prototype's assumptions didn't match, which is why several small frontend adaptations were needed alongside the fetch-call swap itself.
  - **Tested:** Live tested (local MariaDB + PHP built-in server), 45 cases against the exact request/response shapes this JS relies on: login/logout/`auth/me` (success + wrong password 401 + logged-out 401), category list/show/create/update/delete incl. slug de-dup and the 409 delete-in-use guard, public post list/show/category-filter/view-count incl. 404 for drafts and unknown ids, comment submit → hidden-until-approved → visible-after-approve, duplicate-comment 429, validation 422s throughout, and media upload/list/delete with a real image round-trip (fetchable at its returned URL). Found and fixed one real bug this way: `adminStore` requires `author_id` on every create, which the dashboard's post-save payload wasn't sending. Every DOM id `app.js`/`components.js` reference was also cross-checked against the actual HTML (`getElementById`/`querySelector` targets all present) since no headless browser is available in this environment to execute the JS directly.
- [x] Dashboard wired to API — `assets/js/dashboard.js`, `dashboard/login.html`
  - **What:** The dashboard had no login screen at all — added `dashboard/login.html` (posts to `/auth/login`, redirects to `?redirect=`) since a real session is required to call any dashboard endpoint. `api.js`'s `requireDashboardAuth()` runs first on every dashboard page: resolves with `/auth/me`'s user, or redirects to `login.html` on a 401. `dashboard.js` is rewritten to be role-aware: an admin gets `/admin/posts`, `/admin/categories`, `/admin/media`; a non-admin author only ever gets their own posts via `/author/posts` (`/admin/*` categories/media are backend-admin-only, so those two pages show a plain "admin access required" state for an author instead of erroring). Overview stats and the posts table are computed client-side from the fetched posts (no monthly-breakdown or stats endpoint exists on the backend, so the views chart shows a plain note instead of fabricated data). The post editor's slug field is now read-only (the backend always generates the slug server-side and ignores any client-supplied one) and the tags field is hidden entirely (`Post::toArray()` has no tags field — nothing to wire it to). Cover image differs by role: an author gets the original drag/drop file widget, uploading via `POST /author/posts/{id}/image` once the post exists; an admin gets a plain URL field instead, since `/author/posts/{id}/image` only accepts uploads for the caller's *own* post and `adminStore`/`adminUpdate` only take `cover_image` as a string. The topbar's hardcoded "Sarah Chen" was replaced with the real logged-in user's name/role plus a working Log Out button (`POST /auth/logout`) on all 5 dashboard pages.
  - **Where:** `assets/js/dashboard.js`, `dashboard/login.html` (new), all 5 `dashboard/*.html` (topbar user block + logout button, `api.js` `<script>` tag), `dashboard/posts.html` (removed the inline mock-category-populating script, same as `blog.html`).
  - **Why:** Same reasoning as the public site — this is Section 9's admin/author endpoints getting an actual UI in front of them for the first time, which is what surfaced the `author_id`-required and file-vs-URL cover-image constraints noted above.
  - **Tested:** Covered by the same 45-case live-test pass above (auth guards, posts/categories/media CRUD, role-gated 403s) plus the `author_id` fix it caught; every dashboard-page DOM id `dashboard.js` references was cross-checked against the actual HTML the same way as the public pages.

## 12. Mock Data Migration (done)
- [x] Review + seed: authors, categories, posts, comments, media, stats — `bin/seed.php`
  - **What:** A CLI seeder that loads the exact dataset `assets/js/mock-data.js` has been driving the frontend from — the 4 `MOCK_AUTHORS`, 5 `MOCK_CATEGORIES`, 12 `MOCK_POSTS`, 3 `MOCK_COMMENTS`, and 8 `MOCK_MEDIA` items — into the real `blog_*` tables, plus a spread of `blog_post_views_daily` rows per published post so the stats table isn't empty. Also seeds one `admin`-role account (`admin@skoolyst.com`) that has no equivalent in the mock data, since the mock prototype never modeled an admin — the dashboard just assumed one — so the admin-only routes now have someone real to log in as. Every author/category/post/comment/media row is looked up by its natural key (email/slug/title) before inserting, so a partial or repeat run only fills in what's missing instead of erroring or duplicating; `--fresh` wipes all `blog_*` content tables (children before parents, respecting FK order) and reseeds from empty.
...
- [x] Remove `mock-data.js` only after API migration is verified
  - **What:** Deleted `assets/js/mock-data.js`. Along the way found it wasn't purely mock content — `formatDate()` and `escapeHtml()` were defined inside it and genuinely used by `app.js`, `components.js`, `dashboard.js`, and `about.html`'s inline script. Since the previous commit had already removed every page's `<script src="mock-data.js">` tag, those two helpers were silently undefined in every already-pushed page — a real bug caught before it reached anyone, not a pre-existing one. Fixed by extracting just those two functions into a new `assets/js/utils.js`, and adding `<script src="assets/js/utils.js">` (or `../assets/js/utils.js` for dashboard pages) immediately before `api.js` on all 11 pages.
  - **Where:** `assets/js/utils.js` (new), `assets/js/mock-data.js` (deleted), all 6 public pages + all 5 `dashboard/*.html` pages (added the `utils.js` script tag), `assets/js/api.js`/`assets/js/components.js`/`assets/js/app.js` (stale load-order/mock-data.js comments corrected).
  - **Why:** Closes out Section 12 — this was the one remaining checkbox, gated on confirming nothing still depended on the file. It genuinely didn't turn out to be a no-op deletion, which is exactly why this checkbox says "only after verified" instead of just deleting it outright when the frontend was first switched over.
  - **Tested:** Live tested (local MariaDB + PHP built-in server): `node -c` syntax-checked every `assets/js/*.js` file. Fetched every one of the 11 pages (200 on all) and parsed each page's actual `<script src>` tags, resolving relative paths, confirming every one — including the new `utils.js` on both public and dashboard pages — returns 200; `assets/js/mock-data.js` itself now correctly 404s. Re-verified `GET /api/v1/authors` still returns all 4 seeded authors (the endpoint `about.html`'s script depends on, unaffected by this change but re-checked anyway since it's the page most exposed to this bug).

## 13. Frontend PHP Conversion & Shared Layout (in progress)
...
**Pre-existing bug found while extracting `partials/header.php` (public site only, not introduced by this conversion, not yet fixed):** the public pages' hamburger icon and several punctuation marks are literal backslash-escape text — `\u2630`, `\u2014`, `\u2019`, `\u00A9` — instead of real Unicode characters or HTML entities, so they render as that literal text in a browser rather than ☰ / — / ' / ©. Carried over verbatim into the new partials rather than silently "fixed" mid-refactor; worth its own small task later (find/replace across the partials once they're the only copy left).

- [x] Resolve the `index.php` naming clash before the public homepage can become `index.php`: the API front controller was already using that filename (root `index.php` — nothing to do with the frontend homepage). Renamed it to `router.php` so `index.html` can later become the real homepage `index.php`.
  - **What:** `index.php` (the API front controller) renamed to `router.php`. `.htaccess`'s rewrite target updated to match. Two stale doc-comment references to the old filename fixed in `core/Router.php` and `core/Database.php` (both were just explaining "see index.php" / "index.php's exception handler" in passing — no functional code referenced the filename). Also added `bin/dev-router.php`, a local-testing-only script that reproduces `.htaccess`'s exact three-rule logic (real file/dir → serve as-is; `/api/v1/...` → `router.php`; else 404) for accurate `php -S host:port -t . bin/dev-router.php` testing — PHP's built-in server's own "router script" mode calls the given script for *every* request including real files, which isn't what `.htaccess` does, so testing directly against `router.php` as the router script was giving false negatives (a real file like `about.php` returning 404) until this existed.
  - **Where:** `index.php` → `router.php` (git rename), `.htaccess`, `core/Router.php`, `core/Database.php`, `bin/dev-router.php` (new, dev-only, not part of the production request path).
  - **Why:** The plan's public-partials checkbox (below) requires the homepage to eventually be `index.php` — but that name was already taken by the API's own entry point. Renaming the API side now, rather than working around it with a differently-named homepage, keeps the URL structure conventional (`/` → `index.php` naturally, no extra rewrite rule needed) and was the option chosen over the alternatives (keep the homepage `.html` forever, or give the homepage a non-standard name).
  - **Tested:** Live tested with `bin/dev-router.php` faithfully reproducing `.htaccess`: `GET /api/v1/health` → 200 with a fresh timestamp (proves `router.php` itself, not a stale cached response), `GET /api/v1/posts?per_page=1` → 200 with real seeded post data, `GET /api/v1/authors` → 200, an unmatched `GET /api/v1/does-not-exist` → 404 (Router's own not-found path still works), and `GET /about.php` (a real file, unrelated to the API) → 200 — confirming the rename didn't disturb either side: real files still bypass the router entirely, and `/api/v1/*` still reaches it correctly.
- [x] Shared partials for the **public site**: `header.php` (`<head>` + top nav) and `footer.php`, included by `index.php`, `blog.php`, `category.php`, `post.php`, `about.php`, `contact.php` (renamed from their current `.html`)
  - **What/Where/Why/Tested:** see the 2026-09-02 change-log entry above — all 6 public pages now require the shared partials; `.html` originals not yet removed (pending the `.htaccess`/link-rewrite task below).
- [x] Shared partials for the **dashboard**: its own `header.php` (`<head>` + topbar) and `sidebar.php`, included by `dashboard/index.php`, `dashboard/posts.php`, `dashboard/post-editor.php`, `dashboard/categories.php`, `dashboard/media.php` — kept separate from the public partials since the two layouts share no markup (no public nav on dashboard pages, no sidebar on public pages). `dashboard/login.php` (renamed from `login.html`) does **not** use these partials — it has no sidebar/topbar, just a centered login form — and must be explicitly excluded from the auth-guard below (checking a session before showing the login page would be a redirect loop for a logged-out visitor).
  - **What/Where/Why/Tested:** see the 2026-09-02 change-log entry above — all 5 dashboard pages now require the shared partials. `dashboard/login.php` (rename + exclusion) not done yet — bundled with the `.htaccess`/link-rewrite task below since it needs the same `.html`→`.php` decision.
- [ ] `dashboard/header.php` adds a server-side session check (`session_start()` + verify a valid blog session before any dashboard markup is sent, `header('Location: login.html'); exit;` if not) — the current client-side-only guard (`requireDashboardAuth()` in `api.js`, runs after the page has already loaded) never leaked real data since that's separately guarded by the API, but this stops even the empty dashboard shell from being served to a logged-out request, and centralizes the check in one place instead of every page's script relying on calling it correctly
- [x] Per-page `<title>`/meta and the page's own body content stay in that page's `.php` file — only the truly repeated chrome (nav/sidebar/footer) moves into a partial
  - **What/Where/Why/Tested:** Held true across all 11 converted pages (verified via the byte-identical diffs in the change-log entries) — every page sets its own `$pageTitle`/`$pageDescription` (public) or `$pageTitle`/`$topbarTitle`/`$topbarSubtitle` (dashboard) and keeps its own body markup; only nav/footer (public) and sidebar/topbar-chrome (dashboard) live in partials.
...
- [x] Convert one page from each layout first (e.g. `about.php` for public, `dashboard/categories.php` for dashboard) as a proof of concept, live-test it renders identically before converting the rest
  - **What:** Two new shared-partial sets and two proof-of-concept pages, alongside (not replacing yet) their original `.html` files. **Public:** `partials/header.php` (`<head>` + top nav, params: `$pageTitle`, `$pageDescription`, `$activeNav`, optional `$extraHead`) + `partials/footer.php` (confirmed byte-identical across all 6 public pages via `diff` before extracting, so it takes no parameters) → `about.php`. **Dashboard:** `dashboard/partials/sidebar.php` (params: `$activeSidebar`) + `dashboard/partials/header.php` (`<head>` through the topbar and the opening `<main>` tag; params: `$pageTitle`, `$topbarTitle`, `$topbarSubtitle`, `$topbarActionHtml` — the one topbar action differs per page, e.g. "+ New Post" vs. post-editor's "← Back to Posts") + `dashboard/partials/footer.php` (closes `</main></div></div>`) → `dashboard/categories.php`. Scripts and any page-specific modal markup stay in the page itself, after `footer.php`, matching how each page already varied there.
  - **Where:** `partials/{header,footer}.php`, `about.php`, `dashboard/partials/{header,footer,sidebar}.php`, `dashboard/categories.php` — all new files, the original `.html` files untouched for now.
  - **Why:** This is the proof-of-concept checkpoint the plan calls for — confirm the partial boundaries actually work for one page of each layout before repeating the pattern across the other 9 pages.
  - **Tested:** Live tested. Rendered each `.php` file directly via `php` CLI and diffed against its original `.html` byte-for-byte (`diff about.html <(php about.php)`, same for categories) — zero-diff match on both after one whitespace fix in the dashboard topbar partial. Re-confirmed over live HTTP (MariaDB + PHP built-in server): `GET /about.php` and `GET /dashboard/categories.php` both 200, and their response bodies are still byte-identical to the original `.html` files. Checked both files' PHP output under `error_reporting=E_ALL` — no warnings/notices (the `?? ''` defaults on `$activeNav`/`$activeSidebar` mean neither partial complains if a page forgets to set them, it just renders with no `active` class).
- [x] Convert the remaining pages once the two proof-of-concept layouts are confirmed working
  - **What/Where/Why/Tested:** see the 2026-09-02 change-log entry above.
- [x] Live-test every converted page after the full conversion: renders identically to its old `.html` version, no broken asset paths (`assets/...` vs `../assets/...` differs between root pages and `dashboard/*` — the partials must account for this), no console errors, all existing JS/API calls still work unchanged
  - **What/Where/Why/Tested:** see the 2026-09-02 change-log entry above — asset paths were carried over unchanged from each original (root pages keep `assets/...`, dashboard pages keep `../assets/...`, already correctly handled by the existing partials from the POC), so no path-related regressions to check for beyond the byte-identical diff already confirming it. No JS files were touched, so no new console-error risk was introduced.
